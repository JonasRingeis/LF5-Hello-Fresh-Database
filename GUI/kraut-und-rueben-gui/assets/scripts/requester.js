...
const ingredientQuery = "SELECT Z.ZUTAT_ID," +
    ...
    " WHERE ZE.ZUTAT_ID = Z.ZUTAT_ID) AS ERNÄHRUNGSTRENDS FROM ZUTAT AS Z" +
    " (SELECT GROUP CONCAT(EX.NAME SEPARATOR ', ')" +
    " FROM ZUTAT_EXTRAS AS ZX" +
    " JOIN EXTRAS AS EX ON ZX.EXTRAS_ID = EX.EXTRAS_ID" +
    " WHERE ZX_ZUTAT_ID = Z_ZUTAT_ID) AS EXTRAS FROM ZUTAT AS Z" +
